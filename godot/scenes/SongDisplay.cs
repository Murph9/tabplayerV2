...
public partial class SongDisplay : VBoxContainer {
    ...
    private void LoadSong() {
        ...
        var imageFile = Path.Combine(SongFileManager.SONG_FOLDER, _folderName, SongFileManager.ALBUM_ART_NAME);
        Texture2D albumImage = new GradientTexture2D() {
        ...
        if (File.Exists(imageFile)) {
            ...
            if (image is not null) {
                albumImage = ImageTexture.CreateFromImage(image);
            }
        }
        GetNode<TextureRect>("AlbumArtTextureRect").Texture = albumImage;
